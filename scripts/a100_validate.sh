...
python3 -m tiny_transformer.benchmarks.model --checkpoint "$run_dir/bf16/last.pt" --device cuda --precision bf16 --op attention=sdpa --prompt-length 32 --new-tokens 16 --output "$run_dir/baseline.json"
TORCH_LOGS="graph_breaks,recompiles" python3 -m tiny_transformer.benchmarks.model --checkpoint "$run_dir/bf16/last.pt" --device cuda --precision bf16 --op attention=sdpa --compile --prompt-length 32 --new-tokens 16 --output "$run_dir/compile.json" 2> "$run_dir/compile.log"
