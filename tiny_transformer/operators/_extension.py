...
@lru_cache(maxsize=1)
def load_rms_norm_extension():
    # Importing the model/reference operators must not require a CUDA toolkit,
    # Ninja, or a C++ compiler. Only the first student CUDA call builds anything.
    import torch

    if not torch.cuda.is_available():
        raise RuntimeError("student rms_norm requires CUDA-enabled PyTorch and a CUDA device")

    from torch.utils.cpp_extension import CUDA_HOME, load

    if CUDA_HOME is None:
        raise RuntimeError("student rms_norm requires a CUDA toolkit with nvcc; set CUDA_HOME")
