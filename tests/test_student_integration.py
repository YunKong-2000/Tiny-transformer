"""Shared embedding/RMSNorm integration: runtime contract, model training and KV cache."""
...
class StudentRuntimeHostTests(unittest.TestCase):
    def test_cpu_rejection_does_not_load_compiler(self):
        # One fresh process checks both public entries and import-time laziness.
        code = """
import sys
import torch
...
class StudentIntegrationCudaTests(unittest.TestCase):
    ...
    def test_model_training_and_cached_inference(self):
        for names in (('embedding',), ('rms_norm',), ('embedding', 'rms_norm')):
            with self.subTest(operators=names), full_precision_matmul():
                ...
                # and all norm calls, both in FP32 and in the model's AMP path.
    ...
    def test_native_grad_guards_and_deterministic_backward(self):
        ...
        for call in (lambda: embedding.embedding_forward(ids, ew), lambda: norm.rms_norm_forward(x, nw, 1e-6)):
